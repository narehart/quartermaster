...
| **Budget visibility** (declared $1 budget + live countdown hook; then declaration-only isolation) | ⚪ null — floor survives | The countdown never fired; an apparent −38% cost/solved (CI included 1) failed its isolation test (declaration-only: 0.94 vs band, twin test failed). A favorable draw caught before it shipped |
